...
from mPA_interpolated import mapk
import metric_learn
...
mPCA=[20,60,100,140,180,220,260,300]
score_test=np.zeros((1,len(mPCA)))
score_rankk=np.zeros((1,len(mPCA)))
score_map=np.zeros((1,len(mPCA)))

for z in range(len(mPCA)):

    print('start!')
#    pca = decomposition.PCA(n_components=mPCA[i])
#    pca.fit(features_train)
#    pca1 = decomposition.PCA(n_components=mPCA[i])
#    pca1.fit(features_valid)
    pca2 = decomposition.PCA(n_components=mPCA[z])
...
#    features_valid_pca = pca1.transform(features_valid)
    ...
    pred_query, errors = clf.fit(features_query_pca, features_gallery_pca)
    arr_label_query = rk.generate(query_idx, pred_query, label_gallery, label_query, camId_query, camId_gallery)
    
    #rank1 test accuracy
#    score_test[0,i] = accuracy_score(arr_label_query[:,0], label_query)
...
#            if (arr_label_query[i,j]==label_query[i]):
#                arr_label_rankk[i]=arr_label_query[i,j]
#                break
#    score_rankk[0,z] = accuracy_score(arr_label_rankk, label_query)
...
#        score_rankk[0,i-1] = accuracy_score(arr_label_rankk, label_query)
    ...
    score_map[0,z] = mapk(actual,predicted,k=10)
